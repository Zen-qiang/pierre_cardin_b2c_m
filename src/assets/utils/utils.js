...
const utils = {
  ...
  get_nowdate_str: function() {
    let date = new Date()
    let tempDate = date.getFullYear() + '-' + this.formateNum(date.getMonth() + 1) + '-' + this.formateNum(date.getDate()) + ' ' + this.formateNum(date.getHours()) + ':' + this.formateNum(date.getMinutes()) + ':' + this.formateNum(date.getSeconds()) + '.000'
    return tempDate
  },
